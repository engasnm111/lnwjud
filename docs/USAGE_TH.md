# คู่มือใช้งาน lnwjud v4.53.0 (ภาษาไทย)
...
> สำหรับผู้ใช้ Windows x64 ที่ใช้ `lnwjud-Setup-4.53.0.exe` หรือ `lnwjud-Portable-4.53.0.exe` **ไม่ต้องติดตั้ง Node.js และไม่ต้องดาวน์โหลด `tunnel-client.exe` เอง** ตัว release รวม private Node.js runtime และ official OpenAI `tunnel-client v0.0.13` มาให้แล้ว
...
- `lnwjud-Setup-4.53.0.exe` หรือ `lnwjud-Portable-4.53.0.exe`
...
1. ดาวน์โหลด `lnwjud-Setup-4.53.0.exe` จาก GitHub Releases
2. ติดตั้งตามปกติ
...
1. ดาวน์โหลด `lnwjud-Portable-4.53.0.exe`
...
เมื่อ recurring Scheduled turn ตื่นขึ้น ต้องเรียก `claim_scheduled_continuation` เป็น **lnwjud action แรกก่อน mutation**. Mainline v4.53 มีผลลัพธ์สำคัญดังนี้: `recurring_acquired` = ได้ lease ใหม่และทำงานต่อโดยใช้ native task ID เดิม; `worker_busy_noop` = มี worker/blocking work อยู่ ให้คืนตัวโดยไม่ mutate และไม่แตะ Scheduled Task; `orphan_probe_noop` = หลักฐาน orphan ยังไม่ครบ ให้รอรอบชั่วโมงถัดไป; `already_claimed` = tick นี้ถูกจัดการแล้ว; `terminal_cleanup_required` = ทำ cleanup เท่านั้น ห้าม resume งาน และต้องทำ exact recurring task ให้ non-runnable; `terminal_noop` = ไม่มีงานให้ทำ. Recurring tick ปกติ **ห้ามสร้าง successor, ห้าม consume task, ห้าม retime cadence**. ทุก mutation ที่ได้ `recurring_acquired` ต้องแนบ `goalLease` token/generation ของ run ปัจจุบัน. Full Bypass ไม่ข้าม durable-goal ownership fence. ถ้า lease ค้างแต่ไม่มี worker จริง ระบบยังใช้ trustworthy two-probe orphan evidence แบบเดิม; unknown evidence ห้าม force-unlock. สำหรับ historical `occurrence=once` เท่านั้นจึงยังมี `acquired`, `successor_required`, `reschedule_required`, consumed receipt และ fresh one-time successor ตาม compatibility path v4.52.x.
