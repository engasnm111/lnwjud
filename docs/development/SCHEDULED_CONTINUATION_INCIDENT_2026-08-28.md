...
- Claim accepts native wake jitter up to 120 seconds early; this is a safety tolerance, not cadence. A firing wake is a consumed one-time ticket: real worker collisions, an expired lease with a running `blocking_job`, unknown blocking-task liveness, or a wake outside the accepted early window retire that ticket and fail closed into one **fresh adaptive** cloud successor. Same-task updates are reserved for still-pending future tasks handled by `expedite_scheduled_continuation`; `reschedule_required` is legacy compatibility only.
