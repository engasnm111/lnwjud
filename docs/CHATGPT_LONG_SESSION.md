...
เมื่อ recurring watchdog ตื่นต้องเรียก `claim_scheduled_continuation` เป็น lnwjud action แรกก่อน mutation. `recurring_acquired` หมายถึงได้ lease ใหม่และทำงานต่อโดย **ใช้ recurring native task ID เดิม**; `worker_busy_noop` หมายถึงมี live worker หรือ blocking work อยู่และต้องคืนตัวโดยไม่ mutate workspace/lease/native task; `orphan_probe_noop` หมายถึงหลักฐาน orphan ยังไม่ครบสองรอบ; `already_claimed` คือ duplicate delivery ของ tick เดิม; `terminal_cleanup_required` คือ cleanup-only และห้าม resume งาน; `terminal_noop` คือไม่มีงานให้ทำ. Ordinary interval wake ไม่สร้าง successor, ไม่ record `consumed`, และไม่ retime cadence. ถ้าพบ lease ค้างแต่ไม่มี worker จริง ยังต้องมี trustworthy no-worker probe สองรอบห่างอย่างน้อย 120 วินาที โดย goal revision, lease generation และ activity sequence ไม่เปลี่ยน, ไม่มี live fenced call และ blocking task ทุกตัว terminal/absent จึง takeover เป็น `orphan_recovered`. สำหรับ historical `occurrence=once` เท่านั้นจึงยังใช้ `acquired`, `successor_required`, `reschedule_required`, fresh one-time successor และ consumed receipt ตาม compatibility path v4.52.x.
...
Timeline อ้างอิง v4.53: worker lease ปกติ 600 วินาที; recurring watchdog ใหม่ที่ไม่ระบุ legacy first-delay จะมี first firing ประมาณ `T+60` แล้ววนทุก 1 ชั่วโมงด้วย native task ID เดิม. ทุก wake เริ่มด้วย claim: `recurring_acquired` -> lease ใหม่และทำงานต่อ, `worker_busy_noop`/`orphan_probe_noop` -> คืนตัวโดยไม่แตะ task, duplicate -> `already_claimed`. ถ้างานเสร็จให้ `finish_goal -> pending_native_cleanup -> delete/disable exact recurring task -> finish_goal again -> get_goal terminal`. ถ้า cleanup ยังไม่เสร็จและรอบชั่วโมงถัดไปยิง ให้ `terminal_cleanup_required` ทำ cleanup-only. Historical one-time rows เท่านั้นที่ยังใช้ successor/consumed/expedite path.
