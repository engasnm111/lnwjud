...
เมื่อ run ปัจจุบันยังไม่เสร็จและผู้ใช้เปิดใช้ rolling continuation ให้ `prepare_scheduled_continuation` checkpoint + จอง successor แบบ **one-time/cloud** ด้วย delay แบบ adaptive 2–25 นาที และสร้าง native ChatGPT Scheduled Task เดียวในแชทเดิม จากนั้นบันทึก `created` receipt พร้อม native task ID และ `runsOn: cloud` ทันที. `dueAt` เป็น canonical absolute instant แต่ VEVENT ที่ส่งให้ native host ต้องมี explicit IANA `TZID` เพื่อป้องกัน host ตีความ UTC `Z` เป็น local wall-clock ผิดเวลา. ถ้า native create ล้มเหลวหรือไม่แน่นอน ให้ record ตามจริงและ worker ที่ยังมี lease ถูกต้องทำ fenced work ต่อได้จนถึง handoff deadline. ตั้งแต่ v4.52.3 ถ้า Native Scheduled Task host unavailable/unsupported หรือคืน `Resource not found` ให้ถือเป็น **scheduler transport degradation**: record `create_failed`, คง durable goal เป็น `active`, ทำงานจริงต่อใน current leased worker และห้ามใช้ `completed`, `failed` หรือ `blocked` เพียงเพื่อหนีจากการสร้าง successor ไม่สำเร็จ. ถ้า host บังคับ turn boundary ก่อน acceptance ทั้งที่ไม่มี confirmed coverage ให้ checkpoint งานจริง + scheduler degradation ตามจริง, release lease ที่ boundary และห้ามอ้าง autonomous handoff/fallback scheduler. ถ้าไม่ระบุ `successorDelayMinutes` ระบบจะ derive จาก durable-goal lease ปัจจุบันและ clamp 2–25 นาที; lease 600 วินาทีปกติจึงได้ successor ราว **10 นาที** ไม่ใช่ fixed +2. Explicit 2–25 นาทีใช้ได้เฉพาะ bounded intent ที่ caller รู้จริง. ก่อนคืน control ต้องมี confirmed cloud successor หรือ goal terminal; ถ้าต้อง expedite ให้ update ได้เฉพาะ future task ที่ยัง pending และใช้ adaptive due time ที่ tool คืนมา ห้าม hard-code +2.
