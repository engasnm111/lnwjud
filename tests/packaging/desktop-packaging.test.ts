...
describe('Windows desktop packaging', () => {
  it('pins the product release to v4.52.2', async () => {
    ...
    expect(rootPackage.version).toBe('4.52.2');
    expect(desktopPackage.version).toBe('4.52.2');
  });
  ...
  it('keeps every workspace package and runtime version aligned', async () => {
    ...
    for (const packagePath of packagePaths) {
      ...
      expect(packageJson.version, packagePath).toBe('4.52.2');
    }
    ...
    expect(ipcContracts).toContain("APP_VERSION = '4.52.2'");
    expect(shared).toContain("APP_VERSION = '4.52.2'");
  });
