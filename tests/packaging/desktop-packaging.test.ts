...
describe('Windows desktop packaging', () => {
  it('pins the product release to v4.52.3', async () => {
    ...
    expect(rootPackage.version).toBe('4.52.3');
    expect(desktopPackage.version).toBe('4.52.3');
  });
  ...
  it('keeps every workspace package and runtime version aligned', async () => {
    ...
    for (const packagePath of packagePaths) {
      ...
      expect(packageJson.version, packagePath).toBe('4.52.3');
    }
    ...
    expect(ipcContracts).toContain("APP_VERSION = '4.52.3'");
    expect(shared).toContain("APP_VERSION = '4.52.3'");
  });
