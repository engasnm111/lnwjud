...
describe('Windows desktop packaging', () => {
  it('pins the product release to v4.55.0', async () => {
    ...
    expect(rootPackage.version).toBe('4.55.0');
    expect(desktopPackage.version).toBe('4.55.0');
  });
  ...
  it('keeps every workspace package and runtime version aligned', async () => {
    ...
    for (const packagePath of packagePaths) {
      ...
      expect(packageJson.version, packagePath).toBe('4.55.0');
    }
    ...
    expect(ipcContracts).toContain("APP_VERSION = '4.55.0'");
    expect(shared).toContain("APP_VERSION = '4.55.0'");
  });
