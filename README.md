...
## Current version: v4.53.0

The v4.53.0 release target and runtime contract contain **231 total MCP tool definitions**,
...
- A recurring wake must call `claim_scheduled_continuation` first. `recurring_acquired` continues work with a fresh lease; `worker_busy_noop` and `orphan_probe_noop` return without workspace mutation or host-task mutation; duplicate delivery is idempotent; `terminal_cleanup_required` performs cleanup only and never resumes goal work.
...
1. Download the latest published installer from
   ...
   Current Windows 10/11 x64 artifacts are `lnwjud-Setup-4.53.0.exe` (recommended installer) and `lnwjud-Portable-4.53.0.exe` (no installation required).
...
If you prefer not to install the app, run `lnwjud-Portable-4.53.0.exe` directly.
...
1. แบบแนะนำ: ดาวน์โหลด `lnwjud-Setup-4.53.0.exe` แล้วติดตั้งตามปกติ
2. ถ้าไม่ต้องการติดตั้ง: ดาวน์โหลด `lnwjud-Portable-4.53.0.exe` แล้วเปิดได้ทันที
...
```text
apps/desktop/dist/installers/lnwjud-Setup-4.53.0.exe
apps/desktop/dist/installers/lnwjud-Portable-4.53.0.exe
```
