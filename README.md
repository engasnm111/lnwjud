...
| 60 | `wsl_fs` | READ | default | operational | service_dispatch | Translate paths and inspect metadata between a registered Windows workspace and WSL without exposing raw \\wsl$ read/write access. Windows-only: on non-Windows hosts every operation reports WSL as unavailable instead of translating paths. |
