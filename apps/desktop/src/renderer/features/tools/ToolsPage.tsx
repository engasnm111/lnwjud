  ...
  return (
    <section className="panel tools-page" aria-labelledby="tools-heading">
      ...
      <div className="tool-filters">
        ...
        <select value={availability} disabled={origin !== 'lnwjud'} onChange={(event) => setAvailability(event.currentTarget.value as 'all' | 'enabled' | 'disabled')} aria-label={locale === 'th' ? 'สถานะเปิดปิด' : 'Availability'}><option value="all">{locale === 'th' ? 'เปิดและปิดทั้งหมด' : 'All availability'}</option><option value="enabled">{locale === 'th' ? 'เปิด' : 'Enabled'}</option><option value="disabled">{locale === 'th' ? 'ปิด' : 'Disabled'}</option></select>
      ...
      {snapshot === null ? <div className="doctor-empty-state"><p>{locale === 'th' ? 'ยังไม่ได้โหลดข้อมูลเครื่องมือ' : 'Tool catalog has not been loaded yet.'}</p></div> : visible.length === 0 ? <div className="doctor-empty-state"><p>{locale === 'th' ? 'ไม่พบเครื่องมือที่ตรงกับตัวกรอง' : 'No tools match the current filters.'}</p></div> : <div className="tool-card-list">{visible.map((item) => {
        ...
        return <article className={`tool-card tool-${item.readiness}`} key={key}><button type="button" className="tool-card-open" onClick={() => setSelectedKey(key)}><span className="tool-status-dot" aria-hidden="true"/><span className="tool-card-main"><span><strong>{item.title}</strong><code>{item.name}</code></span><small>{item.shortDescription}</small>{item.readiness === 'ready' ? null : <small className="tool-card-remediation-hint">↳ {remediationHint(locale, item, remediationById)}</small>}</span><span className="tool-card-meta"><span>{toolReadinessLabel(locale, item)}</span>{item.origin === 'lnwjud' ? <span className={item.effectiveExposed ? 'tool-availability-on' : 'tool-availability-off'}>{toolAvailabilityLabel(locale, item)}</span> : null}<span>{permissionLabel(locale, item)}</span><span>{profileDecisionLabel(locale, item)}</span></span></button>{item.origin === 'lnwjud' ? <div className="tool-card-availability"><ToolAvailabilitySwitch locale={locale} checked={toolControlEnabled(item)} busy={busy} disabled={onSetAvailability === undefined || (!toolControlEnabled(item) && !toolControlCanEnable(item))} blockedLabel={!toolControlEnabled(item) && !toolControlCanEnable(item) ? (locale === 'th' ? 'ตั้งค่าก่อน' : 'Setup first') : undefined} label={item.title} onChange={(enabled) => { void mutateAvailability(item, enabled); }} />{item.userPreference === 'default' ? null : <button type="button" disabled={busy || onResetAvailability === undefined} onClick={() => { void resetAvailability(item); }}>{locale === 'th' ? 'ใช้ค่าเริ่มต้น' : 'Use default'}</button>}</div> : <div className="tool-card-availability tool-card-availability-readonly">{locale === 'th' ? 'จัดการโดย External MCP' : 'Managed by External MCP'}</div>}</article>;
      })}</div>}
...
function permissionLabel(locale: UiLocale, item: ToolCatalogItem): string {
  if (item.origin !== 'external_mcp' || item.declaredPermission !== 'UNKNOWN') return item.declaredPermission;
  return locale === 'th' ? 'Server ไม่ได้ระบุสิทธิ์' : 'Permission not declared';
}
