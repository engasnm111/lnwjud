...
export class ToolCatalogService {
  ...
  async #snapshot(locale: UiLocale, force: boolean): Promise<ToolCatalogSnapshot> {
    ...
    const firstParty = Object.values(catalogDefinitions).map((definition): ToolCatalogItem => {
      ...
      const eccEnabled = !eccAction || this.#options.eccEnabled?.() === true;
      const eccDisabled = eccAction && !eccEnabled;
      const systemEligible = (delivery === undefined || isAdvertisedDeliveryState(delivery)) && codexEnabled && eccEnabled;
      ...
      const readinessState = computeReadiness(requirementResults, profileDecision, delivery, codexDisabled || eccDisabled);
...
function computeReadiness(requirements: readonly RequirementResult[], profileDecision: ToolProfileDecision, delivery: string | undefined, settingsDisabled: boolean): ComputedReadiness {
  if (delivery === 'planned') return { readiness: 'disabled', readinessReason: 'planned', deliveryState: 'planned', available: false };
  if (delivery === 'feature_disabled' || settingsDisabled) return { readiness: 'disabled', readinessReason: 'feature_disabled', deliveryState: 'feature_disabled', available: false };
