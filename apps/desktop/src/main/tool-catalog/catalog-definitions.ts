...
function requirementsFor(name: string, category: ToolCategory): readonly string[] {
  ...
  if (/^wsl_/.test(name)) {
    ids.add('wsl_runtime');
    // WSL is a Windows-specific boundary. Keep the platform requirement in
    // the catalog so non-Windows clients see unsupported_platform rather than
    // an actionable "install WSL" setup state.
  ...
  if (name === 'accessibility') { ids.add('platform_supported'); ids.add('native_accessibility'); }
  if (name === 'computer_use') {
    ids.add('platform_supported');
    ids.add('native_accessibility');
    ids.add('native_input');
    ids.add('native_window');
    ids.add('native_capture');
  }
  if (name === 'ui_target_action') { ids.add('platform_supported'); ids.add('native_accessibility'); ids.add('native_input'); ids.add('native_window'); ids.add('native_capture'); }
  if (/^input_event$/.test(name)) { ids.add('platform_supported'); ids.add('native_input'); }
  if (/^window$/.test(name)) { ids.add('platform_supported'); ids.add('native_window'); }
  if (/^vision/.test(name)) { ids.add('platform_supported'); ids.add('native_capture'); }
  if (name === 'office' || /^office_(ppt|outlook)$/.test(name) || name === 'inspect_workbook' || name === 'docx_merge') { ids.add('platform_supported'); ids.add('native_office'); }
  // Outlook automation is COM-specific and has no portable provider. Keep it
  // visibly unsupported on macOS/Linux instead of showing a misleading
  // dependency/setup state when a different Office app is installed.
  if (name === 'office_outlook') ids.add('platform_windows');
