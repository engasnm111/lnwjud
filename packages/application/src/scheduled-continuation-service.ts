...
function buildTaskUpdateRequest(
  ...
  return {
    ...
    name: `Continue lnwjud goal ${continuation.goalId}`.slice(0, 120),
    prompt: `Wake the current chat for continuation ${continuation.continuationId}, goal ${continuation.goalId}, workspace ${workspaceId}. This update request is valid only while this exact native task is still pending before it fires. Apply taskUpdateRequest.schedule verbatim and record the host-reported absolute dueAt. When the task later fires, call claim_scheduled_continuation first and treat the firing one-time task as consumed transport identity. If claim returns successor_required, create the exact fresh adaptive successor returned by claim; never re-arm the firing task. If that fresh successor create is unavailable/not-found/unsupported, record create_failed truthfully and keep the durable goal active; scheduler transport failure alone is never goal completion, failure, or blockage. If claim returns terminal_noop, let the firing one-time host task return naturally.`,
  };
}
