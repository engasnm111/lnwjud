...
describe('core tool readiness', () => {
  it('tracks one representative contract for every core tool in the complete 263-tool inventory', () => {
    const registry = coreRegistry();
    expect(registry.listAll()).toHaveLength(263);
    ...
    expect(UPGRADE_TOOL_CATALOG).toHaveLength(147);
  ...
  it('keeps the exhaustive success matrix aligned with all 116 core tools', () => {
    const registry = coreRegistry();
